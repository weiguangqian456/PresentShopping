package com.example.presentshopping.utils.process;
...
public class CompressImageManager implements CompressImage {
    ...
    private ArrayList<PhotoWy> images; // 要压缩的图片集合
    private CompressListener listener; // 压缩监听，告知MainActivity
    ...
    private CompressImageManager(Context context, CompressConfig config,
                                 ArrayList<PhotoWy> images, CompressListener listener) {
    ...
    // 从第一张开始，index = 0
    private void compress(PhotoWy image) {
        // 路径为空
    ...
    private void continueCompress(PhotoWy image, boolean bool, String... error) {
