package com.example.presentshopping.mvp.presenter;
...
import com.example.presentshopping.config.base.BasePresenter;
...
import com.example.presentshopping.mvp.model.PublicModel;
import com.example.presentshopping.net.observer.BaseObserver;
import com.example.presentshopping.net.observer.ObserverOnNextListener;
import com.example.presentshopping.net.observer.TaskCallback;
import com.example.presentshopping.net.observer.UploadProgressBack;
import com.example.presentshopping.ui.dialog.CustomDialogNoTitle;
import com.example.presentshopping.ui.dialog.LoadingDialog;
...
public class PublicPresenter extends BasePresenter<PublicContract.View> implements PublicContract.Presenter {
    ...
    public PublicPresenter(Context context, boolean isShowLoadingDialog, String msg) {
        this.pModel = new PublicModel();
    ...
    private void netWorkRequest(int requestType, String url, String contentType, boolean isAddHeaders, Map<String, Object> headers,
        ...
        // 网络请求
        mObservable.as(RxLife.asOnMain((LifecycleOwner) getView().getContext()))//感知生命周期，并在主线程回调
                .subscribe(new BaseObserver<>(new ObserverOnNextListener<String>() {
                    @Override
                        ...
                        try {
                            if (!isHaveCode) {
                                ...
                                if (code == Constant.CODE_Success) {
                                    ...
                                    if (task != null) {
                                        task.onFailure(null, code, msg, false);
                                    } else {
                                        getView().onFailure(null, code, msg, false);
                                    }
                                }
                            }
                        } catch (Exception e) {
                            ...
                            if (task != null) {
                                task.onFailure(e, Constant.Default_INT, e.getMessage(), false);
                            } else {
                                getView().onFailure(e, Constant.Default_INT, e.getMessage(), false);
                            }
                        }
                    }
                    ...
                    public void onError(Throwable e, boolean isNetWorkError) {
                        LogUtils.e("fxx", "      onError       ");

                        if (task != null) {
                            if (isNetWorkError) {
                                // 网络错误 NetworkError
                                task.onFailure(e, Constant.Default_INT, getView().getContext().getResources().getString(R.string.NetworkError), true);
                            } else {
                                task.onFailure(e, Constant.Default_INT, e.getMessage(), false);
                            }
                        } else {
                            if (isNetWorkError) {
                                // 网络错误 NetworkError
                                getView().onFailure(e, Constant.Default_INT, getView().getContext().getResources().getString(R.string.NetworkError), true);
                            } else {
                                getView().onFailure(e, Constant.Default_INT, e.getMessage(), false);
                            }
                        }
...
//        netWorkRequestPost(url, Constant.contentType_json, true, null, "", params, null);
    ...
    public void netWorkRequestPostForm(String url, String key, File file, UploadProgressBack progressBack) {
        netWorkRequestPostForm(url, true, null, null, key, file, null, progressBack, null);
    }

    public void netWorkRequestPostForm(String url, boolean isAddHeaders, String key, File file, UploadProgressBack progressBack) {
        netWorkRequestPostForm(url, isAddHeaders, null, null, key, file, null, progressBack, null);
    }
    ...
    public void netWorkRequestPostForm(String url, Map<String, Object> params, String key, File file, UploadProgressBack progressBack) {
        netWorkRequestPostForm(url, true, null, params, key, file, null, progressBack, null);
    }
    ...
    public void netWorkRequestPostForm(String url, boolean isAddHeaders, Map<String, Object> params, String key, File file, UploadProgressBack progressBack) {
        netWorkRequestPostForm(url, isAddHeaders, null, params, key, file, null, progressBack, null);
    }
